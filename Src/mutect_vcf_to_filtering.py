...
parser.add_argument('--min-var-count', type = int, default = 1, help = 'Minimum number of variant-supporting reads', dest = 'varcount')
parser.add_argument('--min-depth', type = int, default = 1, help = 'Minimum read depth required across the variant site', dest= 'mindepth')
parser.add_argument('--min-var-frac', type = float, default = 0.0, help = 'Minimum variant allele frequency (0 to 1)', dest= 'vaf')

parser.add_argument('--min-depth-normal', type = int, default = 1, help = 'Minimum read depth required across the variant site in normal', dest='refdepth')

args = parser.parse_args()


# 11 parser.add_argument('--min-var-count', type = int, default = 3, help = 'Minimum number of variant-supporting reads', dest = 'varcount')
# 12 parser.add_argument('--min-depth', type = int, default = 10, help = 'Minimum read depth required across the variant site', dest= 'mindepth')
# 13 parser.add_argument('--min-var-frac', type = float, default = 0.07, help = 'Minimum variant allele frequency (0 to 1)', dest= 'vaf')
# 14 
# 15 parser.add_argument('--min-depth-normal', type = int, default = 8, help = 'Minimum read depth required across the variant site in normal', dest='refdepth')
...
def makeVcfAnnovar(line):
    ...
    if not line.startswith("#"):
        ...
        normal_reads1 = normalInfo.split(":")[1].split(',')[0]
        normal_reads2 = normalInfo.split(":")[1].split(',')[1]
        n_depth = int(normal_reads1) + int(normal_reads2)
        normal_var_freq = round(float(normal_reads2) / float(n_depth), 4)
        ...
        tumor_reads1 = tumorInfo.split(":")[1].split(',')[0]
        tumor_reads2 = int(tumorInfo.split(":")[1].split(',')[1])
        t_depth = int(tumor_reads1) + int(tumor_reads2)
        tumor_var_freq = round(float(tumor_reads2) / float(t_depth), 4)
        vaf = tumor_var_freq
        ...
        if ((len(ref) == 1) & (len(var) == 1)):
            ...
            if(len(ref) > len(var)):
                ...
                var = '-'
                end = int(position) + (len(ref_tmp) - len(ref)) + (len(ref) - 1)
                start = int(position) + (len(ref_tmp) - len(ref))
            else:
        ...
        if (t_depth > args.mindepth) and (tumor_var_freq >= args.vaf) and (tumor_reads2 > args.varcount) and (n_depth > args.refdepth) and (normal_var_freq <= 0.05):
